...
App({
  onLaunch: function (options) {
    ...
    if (config.RUN_ON_SAAS && swan.getExtConfig) {
      var header = {};
      var ext_vars = swan.getExtConfigSync();
      //小应用特殊头
  ...
  onShow: function (options) {
    ...
    if (swan.onNetworkStatusChange) {
      swan.onNetworkStatusChange(function (is_connected, networkType) {
        if (!is_connected) {
          swan.navigateTo({
            url: '/pages/common/noconnect'
          });
...
//        return;
  ...
  getOceanMemeber: function (params) {
    //是否需要member信息
    let ocean_member = swan.getStorageSync('ocean_member');
    if (!ocean_member) {
