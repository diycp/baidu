...
function headers(header) {
    header = header || {};
    //小应用特殊头
    header['X-Requested-isBDAPP'] = 'YES';

    //session_id , vmc_uid 处理
    var session_id = swan.getStorageSync('_SID'),
        vmc_uid = swan.getStorageSync('_VMC_UID');
    var ownshop = swan.getStorageSync('own_shop');
    console.log(ownshop);
    if (config.IS_OCEAN) {
    ...
    if (swan.getStorageSync('merchant_id')) {
        header['X-WxappStorage-MCH-ID'] = swan.getStorageSync('merchant_id');
    }
    if (config.RUN_ON_SAAS) {

        header['x-requested-saas-mode'] = 'APP';

        var ext_vars = false;
        if (swan.getExtConfigSync) {
            ext_vars = swan.getExtConfigSync();
        }

        var host_pre = ext_vars.host_pre || '';
        var client_id = ext_vars.client_id || '';
        var order_id = ext_vars.order_id || '';

        if(host_pre == ''){
            host_pre = config.host_pre || '';
        }
        ...
        header['x-requested-saas-app'] = host_pre;
        header['x-requested-saas-client'] = client_id;
        header['x-requested-saas-order'] = order_id;
...
function wxRequest(params) {
    ...
    var always = function (res) {
        var x_wxappstorage = res.header['X-WXAPPSTORAGE'] ? res.header['X-WXAPPSTORAGE'] : (res.header['x-wxappstorage'] ? res.header['x-wxappstorage'] : false);
        if (x_wxappstorage) {
            var arr = x_wxappstorage.split(';');
            for (var i = 0; i < arr.length; i++) {
                var kv = arr[i].split('=');
                if (kv[0] && kv[1]) {
                    swan.setStorageSync(kv[0], kv[1]);
                } else if (kv[0]) {
                    swan.removeStorageSync(kv[0]);
                }
            }
        }
...
function checkMember(callback, is_retry) {
    ...
    app.getMemberTimer = setTimeout(function () {
        ...
        app.getMember(function (member) {
            ...
            wxRequest({
                url: config.BASE_URL + '/openapi/toauth/callback/bdapp_toauth_pam/callback',
                method: 'POST',
                data: member,
                success: function (res) {
                    if (!res.data.result || res.data.result != 'success') {
                        ...
                        //埋点-身份信息缓存
                        swan.setStorageSync('ocean_member', res.data.data);
...
function getRegionData(step = [0, 0], callback) {
    if (app.globalData.region_data) {
        ...
        if (config.RUN_ON_SAAS && config.app_type) {
            url = config.BASE_HOST + '/__SAAS/__APPS/' + config.app_type + '/' + config.app_year + '/' + config.app_sec + '/data/misc/region_data.js?';
        }
...
function loadImage(page_ins, ident, size) {
    ...
    page_ins.load_image_timer[size] = setTimeout(function () {
        let image_ids = page_ins.image_ids[size].join(',');
        wxRequest({
...
function merchantShare(url) {
    let merchant_id;
    if (swan.getStorageSync('merchant_id')) {
        merchant_id = swan.getStorageSync('merchant_id');
    } else {
        merchant_id = 0;
    }
    if (url.indexOf("?") != -1) {
