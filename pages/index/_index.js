...
var page_options = {
    ...
    onShow: function () {
      if (swan.getStorageSync('is_reload')=='true'){
    ...
    onLoad: function (options) {
        if (options && options.merchant_id) {
          if (options.merchant_id != 0) {
            swan.setStorageSync('merchant_id', options.merchant_id);
          }
        }
        this.setData({
            "options": options
        ...
        const vshop_id = swan.getStorageSync('_vshop_id');
        const merchant_id = swan.getStorageSync('merchant_id');
        ...
        util.wxRequest({
            ...
            success: function (res) {
                ...
                if (pagedata.vshop && pagedata.vshop.shop_id){
                  swan.setStorageSync('own_shop', pagedata.vshop.shop_id)
                }else{
                  swan.removeStorageSync('own_shop');
                }
                swan.setNavigationBarTitle({
                    title: pagedata.vshop?pagedata.vshop.name+'的店铺':pagedata.title
                });
                if (swan.setNavigationBarColor) {
