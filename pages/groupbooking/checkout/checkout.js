...
var update_checkout = function (op) {
    var _this = this;
    var q = '', reqUrl = '', addr_id = '', dly_id = '', pay_id = '';
    ...
    if (op.main_id != undefined) {
        reqUrl = config.BASE_URL + '/m/gbcheckout-' + op.activity_id + '-' + op.product_id + '-' + q + '-' + op.main_id + '.html?addr_id=' + addr_id + '&dlytype_id=' + dly_id + '&payapp_id=' + pay_id;
    } else {
        reqUrl = config.BASE_URL + '/m/gbcheckout-' + op.activity_id + '-' + op.product_id + '-' + q + '.html?addr_id=' + addr_id + '&dlytype_id=' + dly_id + '&payapp_id=' + pay_id;
    }
    util.wxRequest({
        url: reqUrl,
        success: function (res) {
            ...
            _this.setData({
                quantityVal: pagedata.activity_result.quantity
            })
        },
        fail: function (re) {
            console.info(re);
...
var get_submit_data = function (pagedata) {
    ...
    var _vshop_id = swan.getStorageSync('_vshop_id');
    if (_vshop_id) {
        //微店关系
        _return['_vshop_id'] = _vshop_id;
    }
    if (swan.getStorageSync('_qrcode')) {
        //O2O分销,门店扫码关系
        _return['qrcode'] = swan.getStorageSync('_qrcode');
    }
    return _return;
...
Page({
    ...
    onShow: function () {
        //地址变更处理
        var _this = this;
        var addr_selected = swan.getStorageSync('member_addr_selected');
    ...
    evt_showmodal: function (e) {
        ...
        this.setData(_set);
        this.animation = this.animation ? this.animation : swan.createAnimation({
            duration: 400,
            timingFunction: 'ease',
        });
    ...
    evt_submit_order: function (e) {
        ...
        util.wxRequest({
            url: config.BASE_URL + '/m/gbcheckout-create.html',
            data: submit_data,
            method: 'POST',
            success: function (res) {
                if (res.data.error) {
                    ...
                    swan.redirectTo({
                      url: '/pages/groupbooking/payment/payment?order_id=' + order_id + '&flow_success=1&is_gb=true' + '&activity_id=' + pagedata.activity_id + '&product_id=' + pagedata.product_id + '&imgsrc=' + pagedata.imgsrc
                    });
    ...
    tappqminus: function (e) {
        ...
        update_checkout.call(this, {
            ...
            quantity: _this.data.quantityVal
        })
    },
    tappqplus: function (e) {
        var _this = this;
        if (this.data.quantityVal >= this.data.activity_result.member_restrict_number) {
            return;
        }
        this.setData({
