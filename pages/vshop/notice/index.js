...
var load_list = function(page) {
    ...
    util.wxRequest({
        url: config.BASE_URL + '/m/vshop-notice_list-'  + page + '.html',
        success: function(res) {
            var newdata = res.data;
            var _thisdata = _this.data;
            for (var i = 0; i < newdata.notice_list.length; i++) {
                newdata.notice_list[i].last_modify = dateFormat(parseInt(newdata.notice_list[i].last_modify)*1000,'yyyy-mm-dd HH:MM:ss');
            }
            if (newdata) {
